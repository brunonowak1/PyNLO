...
function [sys, signal, fiber, pumps] = Fiber_Prop_Lz_v1(sys, signal, fiber, pumps, step_size)
    ...
    if ~isfield(sys,'frep'), sys.frep = 10*MHz; end % Default repetition rate 
    ...
    if ~isfield(fiber,'NACore'),fiber.NACore = 0.11; end % Core NA
    if ~isfield(fiber,'aCore'), fiber.aCore = 6*um/2; end % Core Radius
    ...
    if strcmpi(fiber.Type,'gain')
        ...
        if ~isfield(fiber,'tau'),fiber.tau = 0.86*ms; end % Relaxation rate
